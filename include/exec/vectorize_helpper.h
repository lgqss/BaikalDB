...
public:
    ...
    static std::shared_ptr<arrow::Schema> get_arrow_schema(const std::vector<pb::TupleDescriptor*>& tuples) {
        std::vector<std::shared_ptr<arrow::Field>> fields;
        fields.reserve(5);
        ...
        if (filter_mask->is_scalar()) {
            ...
            if (mask_scalar.is_valid && mask_scalar.value == true) {
                *out = record_batch;
                ...
                auto empty_batch = arrow::RecordBatch::MakeEmpty(record_batch->schema(), arrow::default_memory_pool());
                if (!empty_batch.ok()) {
        ...
        for (const auto& field : schema->fields()) {
            ...
            auto array_ret = arrow::MakeArrayFromScalar(
                field_value.type == pb::NULL_TYPE ? 
                    ...
                    arrow::LargeBinaryScalar(arrow::Buffer::FromString(field_value.get_string())), 
                1);
            if (!array_ret.ok()) {
    ...
    static std::shared_ptr<arrow::Schema> construct_arrow_schema(const SmartTable table_info) {
