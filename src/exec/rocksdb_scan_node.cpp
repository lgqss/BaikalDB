...
int RocksdbScanNode::choose_index(RuntimeState* state) {
    ...
    if (use_vector) {
        ...
        std::vector<ExprNode*> tmp_scan_conjuncts;
        for (auto expr : _scan_conjuncts) {
            if (expr->contains_specified_fields(_internal_fields)) {
                // 包含__weight的表达式不下推到向量索引
                tmp_scan_conjuncts.emplace_back(expr);
            } else {
                expr->set_is_vector_index_use(true);
                _vector_filter_conjuncts.emplace_back(expr);
            }
        }
        _scan_conjuncts.swap(tmp_scan_conjuncts);
...
bool RocksdbScanNode::need_pushdown(ExprNode* expr) {
    ...
    if (index_type == pb::I_VECTOR) {
        // 向量索引需要将过滤条件下推，用于前过滤
        if (expr != nullptr && expr->contains_specified_fields(_internal_fields)) {
            // 包含__weight的表达式不下推到向量索引
            return false;
        }
        return true;
    ...
    for (auto& slot : _tuple_desc->slots()) {
        ...
        if (pri_field_ids.count(slot.field_id()) == 0) {
            ...
            if (field->short_name == "__weight" ||
                ...
                _internal_fields.insert(field->id);
            }
        }
    }
