...
int LogicalPlanner::create_window_expr(const parser::WindowFuncExpr* item, pb::Expr& expr, const CreateExprOptions& options) {
    ...
    if (create_window_spec(window_spec, *pb_window_spec, options) != 0) {
...
int LogicalPlanner::create_window_spec(
        const parser::WindowSpec* item, pb::WindowSpec& window_spec, const CreateExprOptions& options) {
    if (item == nullptr) {
        DB_WARNING("item is nullptr");
        return -1;
    }
...
int LogicalPlanner::create_expr_tree(const parser::Node* item, pb::Expr& expr, const CreateExprOptions& options) {
    ...
    if (expr_item->expr_type == parser::ET_LITETAL) {
        ...
        if (options.use_alias) {
            ret = create_alias_node(static_cast<const parser::ColumnName*>(expr_item), expr, options.is_group_by);
        }
        if (ret == -2) {
            if (_ctx->stat_info.error_code == ER_ERROR_FIRST) {
            ...
            if (_ctx->stat_info.error_code == ER_ERROR_FIRST) {
                ...
                _ctx->stat_info.error_msg << "Can't group on '"<< expr_item->to_string() << "'";
            }
            return -1;
        } else if (ret == -1) {
...
int LogicalPlanner::create_alias_node(const parser::ColumnName* column, pb::Expr& expr, bool is_group_by) {
    if (!column->db.empty()) {
