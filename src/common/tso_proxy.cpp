...
DEFINE_int64(batch_tso_interval_ms, 3, "batch tso wait interval in ms, [0,10]ms");
...
int TsoProxy::get_tso(const pb::TsoRequest& request, pb::TsoResponse& response) {
    ...
    BthreadCond cond;
    TsoRequestTask request_task(&request, &response, &cond);
...
int TsoProxy::process_response(const std::vector<TsoRequestTask>& requests, const pb::TsoResponse& response) {
    ...
    bool success = !response.has_errcode() || response.errcode() == pb::SUCCESS;
    if (success) {
