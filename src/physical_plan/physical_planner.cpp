...
int PhysicalPlanner::execute_mpp(QueryContext* ctx, DataBuffer* send_buf) {
    ...
    for (auto& [fragment_id, fragment] : ctx->fragments) {
        ...
        if (fragment_id > 0) {
            ...
            if (ret != 0) {
                DB_FATAL("mpp fragment:%d new state init fail", fragment->fragment_id);
                return ret;
            }
        ...
        if (fragment->root == nullptr || fragment->runtime_state == nullptr) {
            DB_FATAL("mpp fragment root or runtime state is nullptr, log_id: %lu, idx: %d", fragment->log_id, fragment_id);
            return -1;
        }
        fragment->runtime_state->use_mpp = true;
        ret = fragment->root->open(fragment->runtime_state); 
        if (ret != 0) {
            DB_FATAL("mpp fragment:%d open fail, ret: %d", fragment_id, ret);
            return ret;
        }
        ret = fragment->root->build_arrow_declaration(fragment->runtime_state);
        if (ret != 0) {
            DB_FATAL("mpp fragment:%d build arrow declaration fail", fragment_id);
            return ret;
        }
    ...
    for (auto& [fragment_id, fragment] : ctx->fragments) {
        ...
        if (!result.ok()) {
            DB_FATAL("mpp fragment: %d, result: %s", fragment_id, result.status().ToString().c_str());
        }
        if (fragment_id == 0) {
            if (result.ok()) {
