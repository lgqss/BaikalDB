...
void MppAnalyzer::restore_sort_pushdown_for_join(QueryContext* ctx) {
    ...
    for (auto& join : join_nodes) {
        ...
        // 两种情况
        // 1. new_sort_node -> join (outer) -> sort_node(limit) -> join2
        // 2. new_sort_node -> join (outer) -> selectmangager(limit) -> sort_node(limit)
        ExecNode* join_parant = join_node->get_parent();
        sort_node->add_child(join_node);
        join_parant->replace_child(join_node, sort_node.release());
    }
    return;
}
...
int MppAnalyzer::seperate_store_fragment(QueryContext* ctx, 
    ...
    if (static_cast<SelectManagerNode*>(select_manager)->mpp_fetcher_global_index(fragment->last_runtime_state) != 0) {
        DB_FATAL("mpp fetcher global index fail");
        return -1;
    }
    return 0;
}
