...
int VectorIndex::search_vector(
    ...
    return search(txn, faiss_index, pk_info, table_info, search_data, topk, efsearch, records, filter_exprs);
}
...
int VectorIndex::add_to_faiss(SmartFaissIndex faiss_index, 
    ...
    std::shared_ptr<arrow::RecordBatch> arrow_record_batch = 
            VectorizeHelpper::construct_arrow_record_batch(faiss_index->scalar_data->schema(), record);
...
int VectorIndex::search(
    ...
    bool has_filter = !filter_exprs.empty();
    std::vector<arrow::compute::Expression> arrow_filter_sub_exprs;
    if (has_filter) {
        arrow_filter_sub_exprs.reserve(filter_exprs.size());
        for (auto* filter_expr : filter_exprs) {
            if (filter_expr == nullptr) {
                DB_WARNING("filter_expr is nullptr");
                return -1;
            }
            if (!filter_expr->can_use_arrow_vector()) {
                DB_WARNING("filter_expr not support arrow vector");
                return -1;
            }
    ...
    int64_t search_count = k;
    {
        ...
        if (_is_flat) {
            ...
            search_params.need_brute_force = true;
            std::shared_ptr<IDSelectorBitmap> bitmap_sel;
            int64_t prefilter_cost = 0;
            if (has_filter) {
                TimeCost tm;
            ...
            DB_WARNING("separate_value: %lu, search:%ld, faiss.ntotal:%lu,%lu, "
                        "search_cnt: %ld,%ld, prefilter_time_cost: %ld, %ld, efsearch: %d", 
                        faiss_index->separate_value, mink, faiss_index->flat_index->ntotal, faiss_index->index->ntotal, 
                        flat_search_cnt, search_cnt, flat_prefilter_cost, prefilter_cost, search_params.efSearch);
...
int VectorIndex::init_faiss_index(SmartFaissIndex faiss_index) {
    ...
    auto arrow_schema = VectorizeHelpper::construct_arrow_schema(table_info);
...
int VectorIndex::restore_faiss_index(const pb::RegionInfo& region_info, SmartFaissIndex faiss_index) {
    ...
    for (iter->Seek(key); iter->Valid(); iter->Next()) {
        ...
        if (value_slice[0] == V_NORMAL) {
            ...
            bthread::RWLockWrGuard lock(faiss_index->rwlock);
            if (mark_del_bitmap(faiss_index, faiss_idx)) {
                DB_WARNING("Fail to mark_del_bitmap");
                return -1;
            }
        } else {
