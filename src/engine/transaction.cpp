    ...
    if (reverse_cols_records != nullptr) {
        ...
        for (const auto& [field_id, schema_info]: fields) {
            if (schema_info->short_name == "__weight") {
                ...
                weight_desc = reverse_record->get_field_by_tag(field_id);
            } else if (schema_info->short_name == "__querywords") {
                querywords_slot_id = field_slot[field_id];
                querywords_desc = reverse_record->get_field_by_tag(field_id);
            }
        }
    }
            ...
            if (use_memrow) {
                ...
                if (reverse_records_vec_ptr != nullptr) {
                    ...
                    if (weight_desc && weight_slot_id != -1) {
                        ...
                        if (field == nullptr) {
                            DB_FATAL("get_tmp_field_value fail: %d, %d", tuple_id, weight_slot_id);
                            return -1;
                        }
                    ...
                    if (querywords_desc && querywords_slot_id != -1) {
                        ...
                        if (field == nullptr) {
                            DB_FATAL("get_tmp_field_value fail: %d, %d", tuple_id, querywords_slot_id);
                            return -1;
                        }
